...
export default function ServicesPage() {
  ...
  return (
    ...
    >
      ...
      <motion.div
        aria-hidden
        className="pointer-events-none absolute inset-0 -z-50" // 👈 moved further behind
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ duration: 1.2 }}
      >
        <div className="absolute -top-24 right-0 h-56 w-56 sm:h-72 sm:w-72 rounded-full bg-[#dff3e6]/60 blur-3xl" />
        <div className="absolute top-[35%] left-0 h-60 w-60 sm:h-80 sm:w-80 rounded-full bg-[#e2efff]/60 blur-3xl" />
        <div className="absolute bottom-10 right-1/4 h-48 w-48 sm:h-64 sm:w-64 rounded-full bg-[#fef3e6]/60 blur-3xl" />
      </motion.div>
      ...
      <div className="max-w-7xl mx-auto w-full relative z-10 pt-28">
        ...
        <motion.p
          ...
          className="text-slate-600 mb-12 text-base sm:text-lg max-w-3xl mx-auto md:mx-0 text-center md:text-left font-medium"
        >
          {typedText}
          <motion.span
            animate={{ opacity: [1, 0] }}
        ...
        <section className="mt-10 sm:mt-16 relative z-[1000]">
          ...
          >
            Our Nutrition & Weight Loss Plans
          </motion.h2>
          {/* --- Responsive Grid --- */}
          <motion.div
          ...
          >
            {plans.map((plan, i) => (
              <motion.div
                ...
                className="w-full max-w-md relative z-[1002]"
              >
                <PlanCard {...plan} />
              </motion.div>
            ))}
